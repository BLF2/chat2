...
import com.wlld.myjecs.bean.AiReq;
...
public class AiController {
    ...
    public Response talk(@RequestBody AiReq aiReq) throws Exception {
        Response response;
        if (Config.starModel) {
            if (Config.start) {
                response = business.talk(aiReq.getSentence());
            } else {
    ...
    public Response myTalk(@RequestBody AiReq aiReq) throws Exception {
        Response response;
        if (Config.starModel) {
            if (Config.start) {
                response = business.myTalk(aiReq.getSentence());
            } else {
                response = new Response();
                response.setError(ErrorCode.ModelNotStart.getError());
                response.setErrorMessage(ErrorCode.ModelNotStart.getErrorMessage());
